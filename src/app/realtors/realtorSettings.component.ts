...
@Component({
    selector: 'app-realtorsettings',
    templateUrl: './realtorSettings.component.html'
})

export class RealtorSettingsComponent {
    ...
    ngOnInit() {
        ...
        this.userService.getUserDetails2().subscribe(data=>{
    ...
    onRSubmit(){
        this.userService.editRealtorReferral(this.i.name.value, this.i.phone_number.value, this.i.email.value, localStorage.getItem('FRLS-D').toString())
        .subscribe(
            data => {
                if(data.status == 200){
    ...
    onNOKSubmit(){
        this.userService.editRealtorNextOfKin(this.j.name.value, this.j.address.value, this.j.phone_number.value, this.j.email.value, localStorage.getItem('FRLS-D').toString())
        .subscribe(
            data => {
                if(data.status == 200){
